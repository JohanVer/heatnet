...
def fuseModule2(channel_in, channel_out, norm_act):
    layers = [("conv1", nn.Conv2d(channel_in, channel_in, 3, stride=1, padding=1, bias=False)),
              ("bn1", norm_act(channel_in)),
              ("conv2", nn.Conv2d(channel_in, channel_in, 3, stride=1, padding=1, bias=False)),
              ("bn2", norm_act(channel_in)),
              ("conv3", nn.Conv2d(channel_in, channel_out, 3, stride=1, padding=1, bias=False)),
              ("up", nn.ConvTranspose2d(channel_out, channel_out, 8, stride=2, padding=3, bias=False))
              ]
    return nn.Sequential(OrderedDict(layers))
...
class ResNeXtEncoder(nn.Module):
    def __init__(self,
        ...
        if input_3x3:
            ...
            layers = [
                ("conv1", nn.Conv2d(in_channels, 64, 7, stride=1, padding=3, bias=False)),
                ("pool", nn.MaxPool2d(3, stride=2, padding=1))
            ]
        ...
        self.fuse_seg2 = fuseModule2(256+classes, classes, norm_act)
...
class ResNeXtDecoder(nn.Module):
    ...
    def __init__(self,
                 norm_act=ABN,
                 out_classes=12,
                 last_activation=nn.LogSoftmax()):

        super(ResNeXtDecoder, self).__init__()

        # Upscaling of convolved earlier stages
        self.up_seg_2 = nn.ConvTranspose2d(out_classes, out_classes, 4, 2, 1, bias=False)

        self.fuse_seg = fuseModule(256+out_classes, out_classes, norm_act)
        self.fuse_seg2 = fuseModule2(256+out_classes, out_classes, norm_act)


        # Loss helper
        self.last_activation = last_activation

    def forward(self, encoded, out_4):


        seg = self.up_seg_2(encoded)
        seg = self.fuse_seg2(torch.cat((seg, out_4), dim=1))

        out = self.last_activation(seg)
...
for name, params in _NETS.items():
    ...
    setattr(sys.modules[__name__], net_name, partial(ResNeXtEncoder, **params))
    setattr(sys.modules[__name__], net_name, partial(ResNeXtDecoder, **params))
